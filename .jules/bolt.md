...
## 2026-06-30 - Optimize High-Frequency State Updates with useRef
**Learning:** For high-frequency events (like audio `timeupdate`), avoiding `useState` for tracking progress or time is critical. Since `useState` triggers component re-renders, it leads to excessive rendering cycles (often multiple times a second), causing layout thrashing, decreased responsiveness, and high CPU usage in large components like `TaranasGallery`.
**Action:** Use `useRef` to store DOM references for progress bars and timers, and manually mutate their `.style` or `.textContent` properties inside event handlers to achieve 60FPS updates without invoking React's render loop.
