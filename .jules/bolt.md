...
## 2026-07-08 - MediaSession mutation inside useEffect lint error
**Learning:** Mutating the `navigator.mediaSession.metadata` API directly inside a `useEffect` hook causes a React hook immutability lint error during `npm run lint`.
**Action:** Adding `// eslint-disable-next-line react-hooks/immutability` fixes the issue safely.
