      ...
      <header className={`rounded-2xl transition-all duration-300 overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-white/50 ${isOpen ? 'bg-white/95 backdrop-blur-xl' : 'bg-white/70 backdrop-blur-lg'}`}>
        ...
        >
          <nav className="flex flex-col space-y-1.5 px-6 py-5 text-sm font-semibold">
            <Link onClick={() => setIsOpen(false)} href="/" className="rounded-xl px-4 py-3 text-[#123962] hover:bg-slate-100 transition-colors">Home</Link>
            <Link onClick={() => setIsOpen(false)} href="/taranas" className="rounded-xl px-4 py-3 text-slate-600 hover:text-[#123962] hover:bg-slate-100 transition-colors">Taranas</Link>
            <Link onClick={() => setIsOpen(false)} href="/literature" className="rounded-xl px-4 py-3 text-slate-600 hover:text-[#123962] hover:bg-slate-100 transition-colors">Literature</Link>
            <Link onClick={() => setIsOpen(false)} href="/articles" className="rounded-xl px-4 py-3 text-slate-600 hover:text-[#123962] hover:bg-slate-100 transition-colors">Articles</Link>
            <Link onClick={() => setIsOpen(false)} href="/events" className="rounded-xl px-4 py-3 text-slate-600 hover:text-[#123962] hover:bg-slate-100 transition-colors">Events</Link>
            <Link onClick={() => setIsOpen(false)} href="/lms" className="rounded-xl px-4 py-3 text-slate-600 hover:text-[#123962] hover:bg-slate-100 transition-colors">LMS</Link>
            <div className="pt-3 pb-1 border-t border-slate-100 sm:hidden">
               <Link onClick={() => setIsOpen(false)} href="/ahbab-link" className="block rounded-xl px-4 py-3 text-[#1C7F93] hover:text-[#123962] hover:bg-slate-100 transition-colors">Ahbab Login</Link>
