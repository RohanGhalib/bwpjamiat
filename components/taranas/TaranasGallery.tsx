...
export default function TaranasGallery({ initialTaranas }: { initialTaranas: Tarana[] }) {
  ...
  const handleTimeUpdate = () => {
    if (audioRef.current) {
      ...
      const currentStr = `${mins.toString().padStart(2, '0')}:${secs.toString().padStart(2, '0')}`;
      currentTimeRefs.current.forEach(el => {
        if (el) el.textContent = currentStr;
      });
    }
  };
  // Sync manual DOM mutations on React re-renders
  useEffect(() => {
    handleTimeUpdate();
  });
    ...
    <div className="relative pb-32">
      ...
      {!isFullScreen && (
         ...
         <div 
            ...
            onClick={handleSeek}
         >
            <div ref={(el) => { progressFillRefs.current[1] = el; }} className="h-full bg-[#1C7F93] relative" style={{ width: "0%" }}>
