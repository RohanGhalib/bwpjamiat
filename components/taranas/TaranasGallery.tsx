...
export default function TaranasGallery({ initialTaranas }: { initialTaranas: Tarana[] }) {
  ...
  // Performance optimization: Use refs instead of state for high-frequency timeupdate events to avoid excessive re-renders
  ...
  const handleSeek = (e: React.MouseEvent<HTMLDivElement>) => {
     if (audioRef.current && audioRef.current.duration && progressBarRef.current) {
        ...
        const progressPercent = (newTime / audioRef.current.duration) * 100;
        progressRefs.current.forEach(el => {
          if (el) el.style.width = `${progressPercent}%`;
        });
     }
