             ...
             <button onClick={closeFullScreen} className="flex items-center gap-2 text-white/85 hover:text-white font-bold text-xs sm:text-sm bg-white/10 hover:bg-white/15 backdrop-blur px-3 sm:px-4 py-2 rounded-full transition-all min-h-10">
                ...
                <button onClick={() => handleDownload(activeTarana)} className="p-2.5 sm:p-2.5 bg-white/10 hover:bg-white/15 text-white/85 hover:text-white rounded-full transition-all backdrop-blur min-h-10 min-w-10" title="Download" aria-label="Download tarana">
                 ...
                 <div className="grid gap-4 sm:gap-6 lg:grid-cols-12 lg:items-start">
                  ...
                  {/* ⚡ Bolt Optimization: Using Next.js Image component instead of native img tag for automatic WebP conversion, responsive resizing, and lazy loading. This significantly reduces LCP and bandwidth usage. */}
                  <div className="w-full max-w-[220px] sm:max-w-md lg:max-w-none lg:w-28 lg:h-28 mx-auto lg:mx-0 aspect-square lg:aspect-auto rounded-[1.5rem] sm:rounded-[3rem] lg:rounded-2xl shadow-2xl relative overflow-hidden mb-4 sm:mb-10 lg:mb-0 border border-white/20 flex items-center justify-center bg-gradient-to-br from-[#123962] to-[#1C7F93]">
                      {activeTarana.coverUrl ? (
                        <Image src={activeTarana.coverUrl} className="w-full h-full object-cover relative z-10" alt="" fill sizes="(max-width: 640px) 220px, (max-width: 1024px) 450px, 112px" />
                      ) : (
                        ...
                        {upNextTaranas.map(t => (
                          ...
                          >
                            <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-white/10 shrink-0 overflow-hidden mr-3 sm:mr-4 shadow-sm border border-white/20">
                              {t.coverUrl ? (
                                <Image src={t.coverUrl} className="w-full h-full object-cover" alt="" fill sizes="48px" />
                              ) : (
             ...
             >
                <div className="flex items-center space-x-4 w-full">
                  <div className="relative w-14 h-14 sm:w-16 sm:h-16 shadow-sm rounded-2xl shrink-0 border border-slate-200 overflow-hidden">
                     {t.coverUrl ? (
                       <Image src={t.coverUrl} alt={t.title} className="w-full h-full object-cover" fill sizes="64px" />
                     ) : (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-0 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:h-24 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
          ...
          >
            <div className="relative w-11 h-11 sm:w-12 sm:h-12 rounded-xl shrink-0 shadow-md overflow-hidden border border-slate-200 group-hover:shadow-lg transition-all group-hover:scale-105">
              {activeTarana?.coverUrl ? (
                <Image src={activeTarana.coverUrl} alt={activeTarana.title} className="w-full h-full object-cover" fill sizes="48px" />
              ) : (
